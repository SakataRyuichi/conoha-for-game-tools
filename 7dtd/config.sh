...
declare -r SDTD_SERVER_DIR="/opt/7dtd/7dtd_server"
declare -r SDTD_SERVER_CONFIG_FILE="$SDTD_SERVER_DIR/serverconfig.xml"
declare -r SDTD_SERVER_SAVE_DIR="/home/sdtd/.local/share/7DaysToDie/Saves"
...
function check_conoha_for_game() {
    if [ ! -d "$SDTD_SERVER_DIR" ] && [ ! -f "$SDTD_SERVER_CONFIG_FILE" ]; then
        die "This server is not a 7 Days to Die Server by ConoHa for GAME."
    fi
}
...
function add_server_admin() {
    ...
    xmlstarlet ed --inplace \
        -s "/adminTools/users" -t elem -n "user" \
        -i "/adminTools/users/user[last()]" -type attr -n "platform" -v "Steam" \
        -i "/adminTools/users/user[last()]" -type attr -n "userid" -v "${steamid64}" \
        -i "/adminTools/users/user[last()]" -type attr -n "name" -v "" \
        -i "/adminTools/users/user[last()]" -type attr -n "permission_level" -v "0" \
        "$SDTD_SERVER_SERVER_ADMINFILE"
...
function add_admin() {
    ...
    while true; do
        read -p "Input user's SteamID64 you want to make an admin: " steamid
        if [ -z "$steamid" ]; then
            break
        elif [[ $steamid =~ ^[0-9]+$ ]]; then
            add_server_admin "$steamid"
            break
        fi
        echo "Invalid SteamID64 format."
    done
}
...
echo "7 Days to Die Server Configutation Tool for ConoHa for GAME"
